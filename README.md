...
1. **Configuration.** Scoring (points or time; a time event also takes a maximum loss in seconds), mulligans (a count, typed in) or Machine Mastery (each machine counts once; ticking it clears the mulligans), whether the evening is prix or single races and whether its lobbies are all public, all private or chosen per slot, and the first slot's time (offsets around the scheduled start, or an exact HH:MM UTC). A first slot outside the scheduled window is allowed with a warning.
